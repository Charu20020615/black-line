...
export default function FormalWears() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const data = await getProducts('formal');
      setProducts(data);
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#d4af37] font-sans">
      ...
      <section className="py-16 px-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold mb-4 gold-text">Formal Wears</h1>
          <p className="text-gray-400 max-w-2xl mx-auto text-lg">
            Explore our premium collection of formal wear, tailored to perfection.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {products.map((product) => (
              ...
              >
                <img
                  src={getImageUrl(product.images?.[0], '/Images/formalwear.png')}
                  alt={product.name}
